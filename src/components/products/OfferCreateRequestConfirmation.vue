  ...
  export default {
    ...
    methods: {
      ...
      accept: function(id, index) {
        var body = {
          url: 'http://teamche.daneshboom.ir/products/offers/' + id  + '/accept/',
          token: this.$cookie.get('teamche_token'),
          method: 'post',
          result: 'acceptProductOffersResult'
        }
        this.$socket.emit('rest request', body)
      ...
      deny: function(id, index) {
        var body = {
          url: 'http://teamche.daneshboom.ir/products/offers/' + id + '/deny/',
          token: this.$cookie.get('teamche_token'),
          method: 'post',
          result: 'denyProductOffersResult'
        }
        this.$socket.emit('rest request', body)
        this.items.splice(index, 1)
      },
