<template>
  <div>
    <v-container>
      <v-card-title>
        <div class="buttons">
          <v-btn color="cyan" dark v-on:click="navigate('/slides/create')">New Slide</v-btn>
        </div>
        <v-spacer></v-spacer>
        <v-text-field
          append-icon="search"
      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td>
            <v-btn flat icon color="orange" class="tools-button" v-on:click="navigate('/slides/update?id=' + props.item.id)">
              <v-icon>edit</v-icon>
            </v-btn>
...
<script>
  export default {
    ...
    created: function() {
      ...
      var body = {
        url: 'http://teamche.daneshboom.ir/base/sliders/?related_parent=null',
    ...
    methods: {
      ...
      deleteRecord: function(id, index) {
        this.$swal({
          ...
          if (result.value) {
            var body = {
              url: "http://teamche.daneshboom.ir/base/sliders/" + id + "/",
              method: 'del',
              token: this.$cookie.get('teamche_token'),
